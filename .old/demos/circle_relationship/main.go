...
func (app *App) Draw(screen *ebiten.Image) {
	...
	// Render the mouse point as a red circle.
	for p := range app.mouseCircle.Bresenham {
		screen.Set(p.X(), p.Y(), colornames.Red)
	}

	// Display mouse position, FPS, and TPS.
	ebitenutil.DebugPrintAt(screen, fmt.Sprintf("Mouse: %s", app.mouseCircle.Center().String()), 5, 224)
...
func (app *App) Update() error {
	...
	app.pointRelationship = app.mouseCircle.RelationshipToPoint(app.point, geom2d.WithEpsilon(1))
	app.lineSegmentRelationship = app.mouseCircle.RelationshipToLineSegment(app.lineSegment, geom2d.WithEpsilon(1e-9))
	app.rectangleRelationship = app.mouseCircle.RelationshipToRectangle(app.rectangle, geom2d.WithEpsilon(1e-9))
	app.circleRelationship = app.mouseCircle.RelationshipToCircle(app.circle, geom2d.WithEpsilon(1))
	app.polyTreeRelationship = app.mouseCircle.RelationshipToPolyTree(app.polyTree, geom2d.WithEpsilon(1))
...
func main() {
	...
	ebiten.SetWindowTitle("geom2d demo: circle relationship to...")
