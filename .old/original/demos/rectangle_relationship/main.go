...
type App struct {
	...
	mouseRectangle geom2d.Rectangle[int] // Current mouse position as a geom2d Rectangle.
...
func (app *App) Draw(screen *ebiten.Image) {
	...
	// Render the mouse point as a red rectangle.
	for _, edge := range app.mouseRectangle.Edges() {
		for p := range edge.Bresenham {
	...
	ebitenutil.DebugPrintAt(screen, fmt.Sprintf("Mouse: %s", app.mouseRectangle.Edges()[0].Start().String()), 5, 224)
...
func (app *App) Update() error {
	...
	app.mouseRectangle = geom2d.NewRectangle([]geom2d.Point[int]{
		geom2d.NewPoint[int](app.mouseX, app.mouseY),
		geom2d.NewPoint[int](app.mouseX+5, app.mouseY+4),
		geom2d.NewPoint[int](app.mouseX, app.mouseY+4),
		geom2d.NewPoint[int](app.mouseX+5, app.mouseY),
	})

	// Update relationships for each geometric object.
	app.pointRelationship = app.mouseRectangle.RelationshipToPoint(app.point, geom2d.WithEpsilon(1))
	app.lineSegmentRelationship = app.mouseRectangle.RelationshipToLineSegment(app.lineSegment, geom2d.WithEpsilon(1e-9))
	app.rectangleRelationship = app.mouseRectangle.RelationshipToRectangle(app.rectangle, geom2d.WithEpsilon(1e-9))
	app.circleRelationship = app.mouseRectangle.RelationshipToCircle(app.circle, geom2d.WithEpsilon(1))
	app.polyTreeRelationship = app.mouseRectangle.RelationshipToPolyTree(app.polyTree, geom2d.WithEpsilon(1))
...
func (app *App) initGeometry() error {
	// Initialize the mouse rectangle.
	app.mouseRectangle = geom2d.NewRectangle([]geom2d.Point[int]{
		geom2d.NewPoint[int](0, 0),
		geom2d.NewPoint[int](5, 4),
		geom2d.NewPoint[int](0, 4),
		geom2d.NewPoint[int](5, 0),
	})
...
func main() {
	...
	ebiten.SetWindowTitle("geom2d demo: rectangle relationship to...")
