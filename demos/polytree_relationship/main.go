...
type App struct {
	...
	mousePolyTree  *geom2d.PolyTree[int] // Current mouse position as a geom2d PolyTree.

	// Single point and its relationship to the mouse point.
	point             geom2d.Point[int]
	pointRelationship map[*geom2d.PolyTree[int]]geom2d.Relationship

	// Line segment and its relationship to the mouse point.
	lineSegment             geom2d.LineSegment[int]
	lineSegmentRelationship map[*geom2d.PolyTree[int]]geom2d.Relationship

	// Rectangle and its relationship to the mouse point.
	rectangle             geom2d.Rectangle[int]
	rectangleRelationship map[*geom2d.PolyTree[int]]geom2d.Relationship

	// Circle and its relationship to the mouse point.
	circle             geom2d.Circle[int]
	circleRelationship map[*geom2d.PolyTree[int]]geom2d.Relationship

	// PolyTree and its relationship to the mouse point.
	polyTree             *geom2d.PolyTree[int]
	polyTreeRelationship map[*geom2d.PolyTree[int]]map[*geom2d.PolyTree[int]]geom2d.Relationship
}
...
func (app *App) Draw(screen *ebiten.Image) {
	...
	ebitenutil.DebugPrintAt(screen, app.pointRelationship[app.mousePolyTree].String(), 5, 20) // Display relationship details.
	...
	ebitenutil.DebugPrintAt(screen, app.lineSegmentRelationship[app.mousePolyTree].String(), 5, 60)
	...
	ebitenutil.DebugPrintAt(screen, app.rectangleRelationship[app.mousePolyTree].String(), 5, 100)
	...
	ebitenutil.DebugPrintAt(screen, app.circleRelationship[app.mousePolyTree].String(), 5, 140)
	...
	ebitenutil.DebugPrintAt(screen, fmt.Sprintf("Polygon: %s", app.polyTreeRelationship[app.mousePolyTree][app.polyTree].String()), 5, 165)
	ebitenutil.DebugPrintAt(screen, fmt.Sprintf("Hole: %s", app.polyTreeRelationship[app.mousePolyTree][app.polyTree.Children()[0]].String()), 5, 180)
	...
	// Render the mouse point as a red polytree.
	for _, edge := range app.mousePolyTree.Edges() {
		for p := range edge.Bresenham {
	...
	ebitenutil.DebugPrintAt(screen, fmt.Sprintf("Mouse: %s", app.mousePolyTree.Edges()[0].Start().String()), 5, 224)
...
func (app *App) Update() error {
	...
	app.mousePolyTree, err = geom2d.NewPolyTree([]geom2d.Point[int]{
		geom2d.NewPoint[int](app.mouseX, app.mouseY),
		geom2d.NewPoint[int](app.mouseX-4, app.mouseY+4),
		geom2d.NewPoint[int](app.mouseX, app.mouseY+8),
		geom2d.NewPoint[int](app.mouseX+4, app.mouseY+4),
	}, geom2d.PTSolid)
	if err != nil {
		return err
	...
	app.pointRelationship = app.mousePolyTree.RelationshipToPoint(app.point, geom2d.WithEpsilon(1))
	app.lineSegmentRelationship = app.mousePolyTree.RelationshipToLineSegment(app.lineSegment, geom2d.WithEpsilon(1e-9))
	app.rectangleRelationship = app.mousePolyTree.RelationshipToRectangle(app.rectangle, geom2d.WithEpsilon(1e-9))
	app.circleRelationship = app.mousePolyTree.RelationshipToCircle(app.circle, geom2d.WithEpsilon(1))
	app.polyTreeRelationship = app.mousePolyTree.RelationshipToPolyTree(app.polyTree, geom2d.WithEpsilon(1))
...
func (app *App) initGeometry() error {
	var err error
	// Initialize the mouse polytree.
	app.mousePolyTree, err = geom2d.NewPolyTree([]geom2d.Point[int]{
		geom2d.NewPoint[int](0, 0),
		geom2d.NewPoint[int](-4, 4),
		geom2d.NewPoint[int](0, 8),
		geom2d.NewPoint[int](4, 4),
	}, geom2d.PTSolid)
	if err != nil {
		return err
...
func main() {
	...
	ebiten.SetWindowTitle("geom2d demo: polytree relationship to...")
